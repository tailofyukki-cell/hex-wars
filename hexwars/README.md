...
**最大5陣営**まで戦える。何陣営で戦うかは**マップが決める**（開幕時にユニットか建物を
持っている陣営だけが参加する）ので、2陣営の既存マップはそのまま動く。

- 勝敗は「敗北していない参加陣営が1つになったら、その陣営の勝ち」
- **ユニットが全滅した陣営はその時点で敗北確定**。確保していた拠点はすべて中立に戻る
  （部隊が居ないのに収入だけ入る状態を残さず、他陣営が採りに行けるようにする）
- 首都を失った陣営も敗北
- 手番は参加していない陣営と脱落した陣営を飛ばす
- フリー対戦の設定画面は参加陣営ごとに「操作（CPU弱/普/強・人間）」と「指揮官」を選ぶ。
  **人間が操作する陣営が1つも無いと開始できない**（誰も操作できない対戦になるため）
- キャンペーンは陣営0が常に人間

`.map` の初期資金は `funds0` 〜 `funds4`。

### チーム

`.map` に `team0` 〜 `team4` を書くと陣営をチームにまとめられる（既定は各自が自分の
チーム＝全軍独立の乱戦）。`leader0` 〜 `leader4` でチームの**主力陣営**を指定する。

```
team0 = 0     # 陣営0 は team0
team2 = 0     # 陣営2 も team0（＝陣営0の援軍）
team1 = 1
team3 = 1
leader0 = 0   # team0 の主力は陣営0
leader1 = 1
```

- **チームの勝敗は主力陣営だけで決まる**。援軍の首都や部隊が残っていても、
  主力が倒れればそのチームの負け。逆に援軍が全滅しても主力が無事なら続行
- 味方・援軍は攻撃できず、援軍の拠点も占領できない
- **視界はチーム内で共有**する
- 自チームのユニットは陣営色のチップに白いリングが付く。ユニット・拠点の情報欄にも
  「（援軍）」「（敵）」が出る

**対戦マップを対称に作るときは 180 度回転を使うこと。** odd-r ヘクスは奇数行が
半マスずれているため、左右反転（`x → W-1-x`）は距離を保たない（実測で約39%の組が
変わる）。見た目は鏡像でも一方が有利になる。
